...
export default function Login({ onLogin }) {
  ...
  const handleDoctor = () => doLogin({ role: "doctor" });
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-xl w-full max-w-4xl flex overflow-hidden" style={{ minHeight: "580px" }}>
        ...
        <div className="w-96 flex flex-col justify-center px-12 py-14">
          ...
          <p className="text-sm text-slate-500 mb-8">
            {adminMode ? "Enter the admin password to continue." : "Choose how you'd like to continue."}
          </p>

          {error && (
            <div className="bg-red-50 border border-red-200 text-red-700 text-xs px-4 py-3 rounded-2xl mb-5">
              {error}
            </div>
          )}
          ...
          {!adminMode ? (
            <div className="space-y-4">
              {/* Continue as Doctor — no password */}
              <button
                type="button"
              ...
              {/* Admin sign in — switches to password mode */}
              <button
                type="button"
                onClick={() => { setError(null); setAdminMode(true); }}
                disabled={loading}
                className="w-full py-3.5 rounded-2xl text-sm font-semibold text-slate-700 border border-slate-200 bg-white hover:bg-slate-50 flex items-center justify-center gap-2 transition-all duration-200 disabled:opacity-50 disabled:pointer-events-none"
              >
                <svg width="16" height="16" viewBox="0 0 20 20" fill="currentColor">
            ...
            <form onSubmit={handleAdmin} className="space-y-5">
              ...
              <button
                type="button"
                onClick={() => { setError(null); setPassword(""); setAdminMode(false); }}
                disabled={loading}
